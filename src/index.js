...
async function add_to_chat(user_id, chat_id, your_id, group_chat_id, group_id, hash) {
    await vk.api.groups
        .unban({
            group_id: group_id,
            owner_id: user_id,
    ...
    await fetch("https://vk.com/al_im.php?act=a_toggle_admin", {
        ...
        body: `act=a_toggle_admin&al=1&chat=${2000000000 + group_chat_id}&gid=${group_id}&hash=${hash}&im_v=3&is_admin=1&mid=${your_id}`,
        method: "POST",
    });
...
function main() {
        let interval = setInterval(async () => {
            if (!(await isUserInChat(chat_id, user_id))) {
                try {
                    await add_to_chat(user_id, chat_id, your_id, group_chat_id, group_id, hash);
                    console.log("added");
                } catch (e) {
                    console.log("catched")
